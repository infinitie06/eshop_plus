...
class ComboProductFaqController extends Controller
{
    ...
    //                 return $query->whereRaw('LOWER(combo_product_faqs.question) LIKE ?', ["%{$search}%"])
    //                     ->orWhereRaw('LOWER(combo_product_faqs.id) LIKE ?', ["%{$search}%"])
    //                     ->orWhereRaw('LOWER(combo_product_faqs.answer) LIKE ?', ["%{$search}%"]);
    //             });
    //     }
        ...
        public function list(Request $request)
    {
        ...
        $product_faqs = $product_faqs->map(function ($p) use ($language_code) {
            ...
            $action = '<div class="dropdown bootstrap-table-dropdown">
                        ...
                        <div class="dropdown-menu table_dropdown product_faq_action_dropdown" aria-labelledby="dropdownMenuButton">
                            <a class="dropdown-item edit-combo-product-faq" data-id="' . $p->id . '" data-bs-toggle="modal" data-bs-target="#edit_modal"><i class="bx bx-pencil"></i> ' . labels('admin_labels.edit', 'Edit') . '</a>
