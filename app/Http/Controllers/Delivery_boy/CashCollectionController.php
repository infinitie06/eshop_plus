...
class CashCollectionController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $baseQuery = Transaction::leftJoin('users', 'transactions.user_id', '=', 'users.id')
            ->leftJoin('orders', 'transactions.order_id', '=', 'orders.id') // JOIN ORDERS
            ->select(
                'transactions.*',
                'users.username as name',
                'users.mobile',
                'users.cash_received',
                'orders.id as order_name' // SELECT ORDER NAME
            )
        ...
        foreach ($searchResults as $row) {
            $formattedRow = [
                ...
                'order_name' => $row->order_name, // ADD THIS
    ...
    public function get_delivery_boy_cash_collection($limit = "", $offset = '', $sort = 'transactions.id', $order = 'DESC', $search = NULL, $filter = "", $laguage_code = 'en')
    {

        $multipleWhere = [];
        $query = Transaction::select('transactions.*', 'users.username as name', 'users.mobile', 'users.cash_received', 'p.id as parcel_id')
            ->leftJoin('users', 'transactions.user_id', '=', 'users.id')
            ->leftJoin('parcels as p', function ($join) {
                $join->on('transactions.order_id', '=', 'p.order_id')
                    ->where('p.active_status', 'delivered');
            })
            ->where('transactions.status', 1);
        ...
        $bulkData['message'] = ($transactions->isEmpty()) ? labels('delivery_labels.cash_collection_not_exist', 'Cash collection does not exist')
            :
            labels('delivery_labels.cash_collection_retrieve_successfully', 'Cash collection are retrieve successfully');
        $bulkData['language_message_key'] = 'cash_collection_not_exist';
        ...
        foreach ($transactions as $row) {
            ...
            $tempRow['parcel_id'] = $row['parcel_id'];
