...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!$is_local_shipping && !$is_standard_shipping) {
            return request()->ajax()
                ? response()->json(['errors' => ['At least one shipping method must be enabled in settings']], 422)
                : redirect()->back()->withErrors(['shipping_method' => 'At least one shipping method must be enabled in settings'])->withInput();
        }
        ...
        if (request()->input('product_type') != 'digital_product') {
            ...
            if ($is_local_shipping) {
                ...
                if (!$is_standard_shipping) {
                    $validator->sometimes('deliverable_type', 'required', fn() => true);
                ...
                if (in_array(request()->input('deliverable_type'), [2, 3, '2', '3'])) {
                    $validator->sometimes('deliverable_zones', 'required|array|min:1', fn() => true);
                    $validator->setCustomMessages(array_merge($validator->customMessages, [
            ...
            if ($is_standard_shipping) {
                ...
                if (!$is_local_shipping || request()->filled('pickup_location')) {
                    // Validate pickup location for standard shipping
                    ...
                    if (request()->input('product_type') == 'simple_product') {
                        // For simple products, validate single set of dimensions
                        $validator->sometimes('weight', 'required|numeric|gt:0', fn() => true);
                        ...
                        // For variable products, validate dimensions for each variant
                        $validator->sometimes('weight.*', 'required|numeric|gt:0', fn() => true);
                        $validator->sometimes('height.*', 'required|numeric|gt:0', fn() => true);
                        $validator->sometimes('breadth.*', 'required|numeric|gt:0', fn() => true);
                        $validator->sometimes('length.*', 'required|numeric|gt:0', fn() => true);
                        ...
                        $validator->setCustomMessages(array_merge($validator->customMessages, [
                            'pickup_location.required' => 'Pickup location is required for standard shipping.',
                            'pickup_location.exists' => 'Selected pickup location is invalid.',
                            'weight.*.required' => 'Weight is required for all variants when using standard shipping.',
                            'weight.*.gt' => 'Weight must be greater than 0 for all variants.',
                    ...
                    if (request()->input('product_type') == 'simple_product') {
                        ...
                        $validator->setCustomMessages(array_merge($validator->customMessages, [
                            'simple_product_stock_status.required' => 'Stock management is required for products using standard shipping. Please enable stock management and provide stock status.',
                        ]));
                    } elseif (request()->input('product_type') == 'variable_product') {
                        // For variable products, check if stock management type is selected
                        $validator->sometimes('variant_stock_level_type', 'required', fn() => true);
                        $validator->setCustomMessages(array_merge($validator->customMessages, [
                            'variant_stock_level_type.required' => 'Stock management is required for products using standard shipping. Please select stock management type (Product Level or Variable Level).',
                        ]));
                    }
                }
            }
        ...
        if ($validator->fails()) {
            ...
            $data = [
                ...
                'minimum_free_delivery_order_qty' => (!$is_standard_shipping && isset($request->minimum_free_delivery_order_qty)) ? $request->minimum_free_delivery_order_qty : '',
                'delivery_charges' => (!$is_standard_shipping && isset($request->delivery_charges)) ? $request->delivery_charges : '',
            ...
            if ($request->product_type == 'simple_product') {
                if (isset($request->simple_product_stock_status) && in_array($request->simple_product_stock_status, array('0', '1'))) {
                    $data['stock_type'] = '0';
                    $data['sku'] = $request->product_sku ?? null;
            ...
            if ($request->product_type == 'simple_product') {
                ...
                Log::info('Variable product creation request data:', [
                ...
                if ($request->has('variants_ids') && !empty($request->variants_ids)) {
                    Log::info('variants_ids received:', [
                        'variants_ids' => $request->variants_ids,
                        'count' => count($request->variants_ids),
                        'is_array' => is_array($request->variants_ids)
                    ]);
                    ...
                    for ($i = 0; $i < count($variants_ids); $i++) {
                        ...
                        if ($flag == "variant_level") {
                            ...
                            $product_variant_data['breadth'] = $variant_breadth[$i] ?? 0;
                            $product_variant_data['length'] = $variant_length[$i] ?? 0;
                            // Ensure SKU and stock are saved if available even in product level
                            $product_variant_data['sku'] = isset($request->sku_variant_type) ? $request->sku_variant_type : '';
                            $product_variant_data['stock'] = isset($request->total_stock_variant_type) ? $request->total_stock_variant_type : '';
                            $product_variant_data['availability'] = isset($request->variant_status) ? $request->variant_status : '';
                            $product_variant_data['status'] = '1';
                        ...
                        // Debug: Log what we're saving for each variant
                    ...
                    Log::warning('variants_ids not found or empty during product creation:', [
                        'has_variants_ids' => $request->has('variants_ids'),
                        'variants_ids_value' => $request->variants_ids,
    ...
    public function list()
    {
        ...
        $query->select('products.id AS id', 'categories.id as category_id', 'brands.id as brand_id', 'products.brand', 'categories.name as category_name', 'brands.name as brand_name', 'seller_store.store_name', 'products.id as pid', 'products.rating', 'products.no_of_ratings', 'products.category_id', 'products.name', 'products.type', 'products.image', 'products.status', 'products.brand', 'product_variants.price', 'product_variants.special_price', 'product_variants.stock')
            ...
            ->leftJoin('seller_store', 'seller_store.seller_id', '=', 'products.seller_id')
        ...
        if (request()->filled('search')) {
            ...
            $query->where(function ($q) use ($search, $language_code) {
                ...
                $q->where('products.id', (string) $search)
                    ->orWhereRaw("LOWER(JSON_UNQUOTE(JSON_EXTRACT(products.name, '$.\"$language_code\"'))) LIKE ?", ["%$search%"])
                    ->orWhereRaw("LOWER(products.description) LIKE ?", ["%$search%"])
                    ->orWhereRaw("LOWER(products.short_description) LIKE ?", ["%$search%"])
        ...
        $total = (clone $query)->get()->unique('pid')->count();

        $products = (clone $query)->groupBy('pid')
            ->orderBy($sort, $order)
            ->skip($offset)
            ->take($limit)
            ->get();
            ...
            $action = '<div class="dropdown bootstrap-table-dropdown">
                    <a href="#" class="text-dark" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        ...
                        <div class="dropdown-menu table_dropdown product_action_dropdown" aria-labelledby="dropdownMenuButton">
                            ...
                            <a class="dropdown-item dropdown_menu_items" href="' . $show_url . '"><i class="bx bxs-show mx-2"></i> ' . labels('admin_labels.view', 'View') . '</a>
                        </div>
                    </div>';
            $variations = '';
    ...
    public function update(Request $request, $data)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($request->product_type == 'simple_product') {
                ...
                // Disable all previous variants (for leftover variants from prev variable state)
                Product_variants::where('product_id', $data)->update([
                    'status' => 0,
                    'availability' => 0
                ]);
                ...
                $simple_variant_data = [
                    ...
                    'weight' => is_array($request->weight) ? ($request->weight[0] ?? 0) : $request->weight,
                    'height' => is_array($request->height) ? ($request->height[0] ?? 0) : $request->height,
                    'breadth' => is_array($request->breadth) ? ($request->breadth[0] ?? 0) : $request->breadth,
                    'length' => is_array($request->length) ? ($request->length[0] ?? 0) : $request->length,
                ...
                // Disable all previous variants (clears any leftover variants from a prior variable state)
                Product_variants::where('product_id', $data)->update([
                    'status' => 0,
                    'availability' => 0
                ]);
                ...
                if (!empty($request->variants_ids) && is_array($request->variants_ids)) {
                    ...
                    for ($i = 0; $i < $count; $i++) {
                        ...
                        if (
                            empty($request->variants_ids[$i]) &&
                            empty($request->variant_price[$i]) &&
                            empty($request->variant_sku[$i]) &&
                            empty($variant_images[$i])
                        ) {
                            continue; // skip empty variant row
                        }

                        $attributeValueIds = str_replace(' ', ',', trim($request->variants_ids[$i]));
                        $submitted_attribute_value_ids[] = $attributeValueIds;

                        $variant_payload = [
                            'attribute_value_ids' => $attributeValueIds,
                            'price' => $request->variant_price[$i] ?? 0,
                            'special_price' => $request->variant_special_price[$i] ?? 0,
                            'weight' => $request->weight[$i] ?? 0,
                            'height' => $request->height[$i] ?? 0,
                            'breadth' => $request->breadth[$i] ?? 0,
                            'length' => $request->length[$i] ?? 0,
                            'sku' => $request->variant_sku[$i] ?? '',
                            'stock' => $request->variant_total_stock[$i] ?? '',
                        ...
                        $existing_variant = Product_variants::where('product_id', $data)
                            ->where('attribute_value_ids', $attributeValueIds)
                            ->orderByRaw('status DESC, id DESC')
                            ->first();

                        if ($existing_variant) {
                            $existing_variant->update($variant_payload);
                            ...
                            $variant_payload['product_id'] = $data;
