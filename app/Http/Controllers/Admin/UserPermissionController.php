...
class UserPermissionController extends Controller
{
    ...
    public function systemUsersList(Request $request)
    {
        ...
        $query = User::whereIn('role_id', [1, 5, 7, 6])
        ...
        $data = $users->map(function ($u) use ($allowModification) {
            ...
            $role_name = $role_name === 'super_admin' ? 'Super Admin' : ($role_name ?? 'Unknown');
    ...
    <div class="dropdown-menu table_dropdown system_user_action_dropdown" aria-labelledby="dropdownMenuButton">';
            ...
            if ($role_name !== 'Super Admin') {
                $action .= '<a class="dropdown-item dropdown_menu_items" href="' . $edit_url . '">
                    <i class="bx bx-pencil mx-2"></i> ' . labels('admin_labels.edit', 'Edit') . '
                </a>
                <a class="dropdown-item delete-data dropdown_menu_items" data-url="' . $delete_url . '">
                    <i class="bx bx-trash mx-2"></i> ' . labels('admin_labels.delete', 'Delete') . '
                </a>';
            } else {
                $action .= '<span class="dropdown-item text-muted">No actions allowed</span>';
            }
            ...
            return [
                'id' => $u->id,
                'username' => $u->username,
                'country_code' => $allowModification ? $u->country_code : '***',
