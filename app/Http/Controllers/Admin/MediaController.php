...
class MediaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Default limit = 25 (your Blade expects 25/50/75)
        ...
        $query = Media::where('store_id', $store_id);
        ...
        // Use paginate to match your Blade
        $media = $query->orderBy($sort, $order)->paginate($limit)->withQueryString(); // keeps search, type, sort, limit in URL
        //dd($media);
        if ($media->isEmpty() && $media->currentPage() > 1) {
            // Redirect to the last valid page
            return redirect()->route('admin.media', array_merge(request()->query(), ['page' => $media->lastPage()]));
        }
    ...
    public function dynamic_image(Request $request)
    {
        ...
        try {
            ...
            $cachePaths = $mediaService->cachedDynamicImagePath($url, $width, $quality);

            // Handle SVG
            if ($mimeType === 'image/svg+xml') {
                $encodedImage = $this->resizeSvg($imageData, $width);
                $this->writeImageCache($cachePaths, $encodedImage);
            ...
            $this->writeImageCache($cachePaths, (string) $encodedImage);
    ...
    private function writeImageCache($cachePaths, $bytes)
    {
        if (!is_array($cachePaths) || empty($cachePaths['absolute'])) {
            return;
        }
        $dir = dirname($cachePaths['absolute']);
        if (!is_dir($dir)) {
            @mkdir($dir, 0775, true);
        }
        @file_put_contents($cachePaths['absolute'], $bytes);
    }
