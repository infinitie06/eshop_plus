...
class Delivery_boyController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->has('edit_id')) {
            ...
            $userData = User::select('id', 'username', 'mobile', 'email', 'country_code', 'address', 'bonus_type', 'bonus', 'serviceable_zones', 'front_licence_image', 'back_licence_image', 'disk')
                ->where('id', $editId)
                ->first();
    ...
    public function store(Request $request, $fromApp = false)
    {
        ...
        $fcm_ids_array = $fcm_ids->pluck('fcm_id')->toArray();
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$delivery_boy_data) {
            ...
            $user_data['country_code'] = $request->country_code ?? $delivery_boy_data->country_code;
