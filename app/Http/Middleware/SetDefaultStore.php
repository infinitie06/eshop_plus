...
class SetDefaultStore
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (!file_exists($sqlDumpPath) && !file_exists($installViewPath)) {
            ...
            if (empty(session('store_id')) && $defaultStore) {
                session()->put('store_id', $defaultStore->id);
                session()->put('store_name', $defaultStore->name);
                session()->put('store_image', $defaultStore->image);
                session()->put('store_slug', $defaultStore->slug);
            }
        }
        return $next($request);
    }
