...
@endsection
@section('content')
    <x-admin.breadcrumb :title="labels('admin_labels.stock_manage', 'Stock Manage')" :subtitle="labels('admin_labels.know_your_stock_grow_your_business', 'Know your stock, grow your business')" :breadcrumbs="[['label' => labels('admin_labels.stock_manage', 'Stock Manage')]]" />
        ...
        <div class="modal-dialog  modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form enctype="multipart/form-data" method="POST" class="submit_form">
                    @method('PUT')
                    @csrf
                    <input type="hidden" class="edit_variant_id" name="edit_variant_id">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select class='form-control form-select' name='type' id='type'>
                                        <option value="add">{{ labels('admin_labels.add_option', 'Add') }}</option>
                                        <option value="subtract">{{ labels('admin_labels.subtract_option', 'Subtract') }}</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 col-lg-6 d-flex justify-content-end">
                            ...
                            <a class="btn me-2" id="tableFilter" data-bs-toggle="offcanvas"
                                ...
                                sellerFilter='true' categoryFilter='true'><i class='bx bx-filter-alt'></i></a>
                            ...
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu" aria-labelledby="exportOptionsDropdown">
                                    <li><button class="dropdown-item" type="button"
                                            onclick="exportTableData('admin_product_stock_table','csv')">{{ labels('admin_labels.csv', 'CSV') }}</button></li>
                                    <li><button class="dropdown-item" type="button"
                                            onclick="exportTableData('admin_product_stock_table','json')">{{ labels('admin_labels.json', 'JSON') }}</button></li>
                                    <li><button class="dropdown-item" type="button"
                                            onclick="exportTableData('admin_product_stock_table','sql')">{{ labels('admin_labels.sql', 'SQL') }}</button></li>
                                    <li><button class="dropdown-item" type="button"
                                            onclick="exportTableData('admin_product_stock_table','excel')">{{ labels('admin_labels.excel', 'Excel') }}</button>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    ...
                    <div class="pt-0">
                        <div class="table-responsive">
                            <table id='admin_product_stock_table' data-toggle="table"
                                ...
                                data-url="{{ route('admin.manage_stock.list', ['flag' => request('flag')]) }}"
