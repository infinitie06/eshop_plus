...
@php
    ...
    $adminPreferences = is_string($settings) ? json_decode($settings, true) : $settings;
    $showOrderNotifications =
        isset($adminPreferences['order_notification']) && $adminPreferences['order_notification'] == 1;
...
@section('content')
    ...
    <div class="col-md-12">
        <div class="row">
            ...
            @if (!$allDone)
                ...
                <div class="stepper">
                    @foreach ($setupSteps as $index => $step)
                        @php
                            $done = $completedSteps[$index] ?? false;
                            $activeIndex = array_search(false, array_values($completedSteps));
                            $active = $index === $activeIndex;
                            $stepNumber = $loop->iteration;
                        @endphp
                        <div class="step {{ $done ? 'completed' : '' }} {{ $active ? 'active' : '' }}">
                            <div class="circle">{{ $done ? '✓' : $stepNumber }}</div>
                            <div class="label">{{ is_array($step) ? $step['label'] : $step }}</div>
                        </div>
                    @endforeach
                </div>

                <p>{{ $completedCount }} of {{ $totalSteps }} steps completed ({{ $progressPercentage }}%)</p>
            @elseif (session('show_done_message'))
                <div class="done-message">
                    <h4>🎉 {{ labels('admin_labels.you_are_all_set', 'You are all set!') }}</h4>
                    <p>{{ labels('admin_labels.all_setup_done_message', 'All setup steps are completed. Your system is ready to go.') }}</p>
                </div>
            @endif
            ...
            <div class="col-md-12 col-xxl-4">
                ...
                @if ($showOrderNotifications)
                    <div class="col-md-12 mb-4">
                        <div class="card">
                            <div class="card-body">
                                <h4>{{ labels('admin_labels.order_notifications', 'Order Notifications') }}</h4>
                                <div class="list-group">
                                    @php
                                        // Fetch recent orders or notifications (e.g., new or pending orders)
                                        $recentOrders = app(OrderService::class)->getRecentOrdersForNotifications(
                                            $store_id,
                                    ...
                                    @forelse ($recentOrders as $order)
                                        <div class="list-group-item d-flex justify-content-between align-items-center">
                                            <div>
                                                <strong>{{ labels('admin_labels.order_id', 'Order ID') }}:
                                                    {{ $order->id }}</strong>
                                                <p class="mb-0">{{ labels('admin_labels.status', 'Status') }}:
                                                    {{ ucfirst($order->active_status) }}</p>
                                                <small>{{ labels('admin_labels.date', 'Date') }}:
                                                    {{ $order->date_added }}</small>
                                            </div>
                                            <a href="{{ route('admin.orders.view', $order->id) }}"
                                                class="btn btn-sm btn-primary">
                                                {{ labels('admin_labels.view', 'View') }}
                                            </a>
                                        </div>
                                    @empty
                                        <div class="list-group-item text-center">
                                            <p>{{ labels('admin_labels.no_notifications', 'No new order notifications') }}
                                            </p>
                                        </div>
                                    @endforelse
                                </div>
            ...
            <!-- Missing Language Labels Section -->
            {{-- @if(isset($missing_labels_summary) && count($missing_labels_summary) > 0)
            <div class="col-md-12 col-xxl-12 mt-md-2 mt-xxl-2">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">
                            <i class="fas fa-exclamation-triangle text-warning me-2"></i>
                            {{ labels('admin_labels.missing_language_labels', 'Missing Language Labels') }}
                        </h5>
                        <a href="{{ route('language.index') }}" class="btn btn-sm btn-primary">
                            {{ labels('admin_labels.manage_languages', 'Manage Languages') }}
                        </a>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-hover">
                                <thead>
                                    <tr>
                                        <th>{{ labels('admin_labels.language', 'Language') }}</th>
                                        <th>{{ labels('admin_labels.file_type', 'File Type') }}</th>
                                        <th>{{ labels('admin_labels.missing_labels', 'Missing Labels') }}</th>
                                        <th>{{ labels('admin_labels.total_missing', 'Total Missing') }}</th>
                                        <th>{{ labels('admin_labels.last_updated', 'Last Updated') }}</th>
                                        <th>{{ labels('admin_labels.action', 'Action') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($missing_labels_summary as $language)
                                        @foreach($language['files'] as $file)
                                            @if($file['missing_labels_count'] > 0)
                                            <tr>
                                                <td>
                                                    <strong>{{ ucfirst($language['language_name']) }}</strong>
                                                    <br>
                                                ...
                                                <td>
                                                    @if(count($file['missing_labels']) > 0)
                                                        <div class="missing-labels-preview">
                                                            @foreach(array_slice($file['missing_labels'], 0, 5) as $label)
                                                                <span class="badge bg-danger me-1 mb-1">{{ $label }}</span>
                                                            @endforeach
                                                            @if(count($file['missing_labels']) > 5)
                                                                <span class="text-muted">+{{ count($file['missing_labels']) - 5 }} more</span>
                                                            @endif
                                                        </div>
                                                        <a href="#"
                                                           class="btn btn-sm btn-link p-0 mt-1 text-primary view-missing-labels"
                                                           data-missing-labels='@json($file['missing_labels'])'
                                                           data-language-name="{{ $language['language_name'] }}"
                                                           data-file-type="{{ $file['file_type'] }}">
                                                            <i class="fas fa-eye me-1"></i>{{ labels('admin_labels.view_all_labels', 'View All Labels') }} ({{ count($file['missing_labels']) }})
                                                        </a>
                                                    @else
                                                ...
                                                <td>
                                                    <span class="badge bg-danger">{{ $file['missing_labels_count'] }}</span>
                                                </td>
                                                <td>
                                                    @if($file['updated_at'])
                                                        {{ \Carbon\Carbon::parse($file['updated_at'])->format('M d, Y H:i') }}
                                                    @else
                                                        <span class="text-muted">Never</span>
                                                    @endif
                                                </td>
                                                <td>
                                                    <a href="{{ route('language.index') }}?lang={{ $language['language_code'] }}&type={{ $file['file_type'] }}"
                                                       class="btn btn-sm btn-outline-primary">
                                                        <i class="fas fa-edit me-1"></i>{{ labels('admin_labels.edit', 'Edit') }}
                                                    </a>
                                                </td>
                                            </tr>
                                            @endif
                        ...
                        <div class="mt-3">
                            <div class="alert alert-info mb-0">
                                ...
                                <strong>{{ labels('admin_labels.note', 'Note') }}:</strong>
                                {{ labels('admin_labels.missing_labels_note', 'Missing labels are compared against the English (en) reference files. Upload updated JSON files to complete translations.') }}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {{-- Full Missing Labels Listing (append-only, keeps existing layout intact) --}}


            <!-- Missing Labels Modal -->
            <div class="modal fade" id="missingLabelsModal" tabindex="-1" aria-labelledby="missingLabelsModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="missingLabelsModalLabel">
                                <i class="fas fa-exclamation-triangle text-warning me-2"></i>
                                {{ labels('admin_labels.missing_labels', 'Missing Labels') }}
                            </h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="mb-3">
                                <strong>{{ labels('admin_labels.language', 'Language') }}:</strong>
                                <span id="modal_language_name"></span>
                            </div>
                            <div class="mb-3">
                                <strong>{{ labels('admin_labels.file_type', 'File Type') }}:</strong>
                                <span class="badge bg-info" id="modal_file_type"></span>
                            </div>
                            <div class="mb-3">
                                <strong>{{ labels('admin_labels.total_missing', 'Total Missing') }}:</strong>
                                <span class="badge bg-danger" id="modal_total_count"></span>
                            </div>
                            <hr>
                            <div class="mb-3">
                                <input type="text" id="search_missing_labels" class="form-control" placeholder="{{ labels('admin_labels.search_labels', 'Search labels...') }}">
                            </div>
                            <div id="missing_labels_list" style="max-height: 400px; overflow-y: auto;" class="border p-3 rounded">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-primary" id="copy_all_labels_btn">
                                <i class="fas fa-copy me-1"></i>{{ labels('admin_labels.copy_all', 'Copy All') }}
                            </button>
                        </div>
                    </div>
                </div>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="d-flex mt-4">
                                        ...
                                        <div class="col">
                                            ...
                                            @php
                                                $currentRecivedOrder = app(OrderService::class)->ordersCount(
                                                    'received',
                                                    '',
                                                    '',
                                                    $store_id,
                                                );
                                                $maxValue = app(OrderService::class)->ordersCount(
                                                    '',
                                                    '',
                                                    '',
                                                    $store_id,
                                    ...
                                    <div class="d-flex mt-5">
                                        ...
                                        <div class="col">
                                            ...
                                            @php
                                                $currentProcessedOrder = app(OrderService::class)->ordersCount(
                                                    'processed',
                                                    '',
                                                    '',
                                                    $store_id,
                                    ...
                                    <div class="d-flex mt-5">
                                        ...
                                        <div class="col">
                                            ...
                                            @php
                                                $currentShippedOrder = app(OrderService::class)->ordersCount(
                                                    'shipped',
                                                    '',
                                                    '',
                                                    $store_id,
                                    ...
                                    <div class="d-flex mt-5">
                                        ...
                                        <div class="col">
                                            ...
                                            @php
                                                $currentDeliveredOrder = app(OrderService::class)->ordersCount(
                                                    'delivered',
                                                    '',
                                                    '',
                                                    $store_id,
                ...
                <div class="col-md-12 col-xxl-12 mt-md-2 mt-xxl-0">
                    <div class="card">
                        <div class="card-body">
                                    ...
                                    <div class="col-md-8 col-4 d-flex justify-content-end">
                                        <div>
                                            ...
                                            <div class="d-flex align-items-center">
                                                <p class="data_total_font m-0">{{ $item['total_commission'] }} {{ labels('admin_labels.sales_count_label', 'Sales') }}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            @empty
                                <div class="d-flex justify-content-center">
                                    <p>{{ labels('admin_labels.no_data_found_text', 'No data found') }}</p>
                                </div>
                            @endforelse
                        </div>
                    </div>
                    ...
                    <div class="card-body">
                        <div class="row align-items-center d-flex heading mb-5">
                            <div class="col-md-12">
                                <div class="row">
                                    <div class="col-md-8">
                                        <h4>{{ labels('admin_labels.order_items', 'Order Items') }} </h4>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-12">
                    ...
                    <div class="modal-body">
                        <div class="form-group ">
                            ...
                            <input type="text" class="form-control" name="courier_agency" id="courier_agency"
                                placeholder="{{ labels('admin_labels.placeholder_courier_agency', 'Courier Agency') }}" />
                        </div>
                        <div class="form-group">
                            <label class="mb-2 mt-2"
                            ...
                            <input type="text" class="form-control" name="tracking_id" id="tracking_id"
                                placeholder="{{ labels('admin_labels.placeholder_tracking_id', 'Tracking Id') }}" />
                        </div>
                        <div class="form-group ">
                            <label class="mb-2 mt-2" for="url">{{ labels('admin_labels.url', 'URL') }}</label>
                            <input type="text" class="form-control" name="url" id="url"
                                placeholder="{{ labels('admin_labels.url_text', 'URL') }}" />
                        </div>
                    </div>
                    <div class="modal-footer">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">
                    ...
                    {{ labels('admin_labels.all_missing_labels', 'All Missing Labels') }}
                </h5>
                <a href="{{ route('language.index') }}" class="btn btn-sm btn-outline-primary">
                    <i class="fas fa-language me-1"></i>{{ labels('admin_labels.manage_languages', 'Manage Languages') }}
                </a>
            </div>
            <div class="card-body">
                <div class="accordion" id="missingLabelsAccordion">
                                ...
                                <div class="accordion-item">
                                    <h2 class="accordion-header" id="heading-{{ $itemId }}">
                                        <button class="accordion-button collapsed" type="button"
                                            ...
                                            <span class="me-2 text-uppercase badge bg-info">{{ $file['file_type'] }}</span>
                                            <strong class="me-2">{{ ucfirst($language['language_name']) }}</strong>
                                            <small class="text-muted">({{ $language['language_code'] }})</small>
                                            <span class="badge bg-danger ms-auto">{{ $file['missing_labels_count'] }}</span>
                                        </button>
                                    </h2>
                                    <div id="collapse-{{ $itemId }}" class="accordion-collapse collapse"
                                         aria-labelledby="heading-{{ $itemId }}"
                                         data-bs-parent="#missingLabelsAccordion">
                                        <div class="accordion-body">
                                            <div class="d-flex align-items-center mb-3 flex-wrap">
                                                ...
                                                <span class="me-3">
                                                    <strong>{{ labels('admin_labels.last_updated', 'Last Updated') }}:</strong>
                                                    {{ $file['updated_at'] ? \Carbon\Carbon::parse($file['updated_at'])->format('M d, Y H:i') : labels('admin_labels.never', 'Never') }}
                                                </span>
                                                <a href="{{ route('language.index') }}?lang={{ $language['language_code'] }}&type={{ $file['file_type'] }}"
                                            ...
                                            <div class="row">
                                                @foreach($file['missing_labels'] as $idx => $label)
                                                    <div class="col-md-6 mb-2">
                                                        <div class="d-flex align-items-center">
                                                            <span class="badge bg-danger me-2">{{ $idx + 1 }}</span>
                                                            <code class="text-break" style="font-size: 0.9em;">{{ $label }}</code>
                                                        </div>
                                                    </div>
                                                @endforeach
                                            </div>
