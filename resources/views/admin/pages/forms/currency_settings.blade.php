                    ...
                    <form id="" action="{{ route('currency_setting.store') }}" class="submit_form"
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label for="">
                                    {{ labels('admin_labels.currency_name', 'Currency Name') }}
                                </label>
                                <input type="text" name="name" class="form-control" id="name"
                                    placeholder="{{ labels('admin_labels.placeholder_india_rupee', 'India Rupee') }}">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label for="">
                                    {{ labels('admin_labels.currency_symbol', 'Currency Symbol') }}
                                </label>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label for="">
                                    {{ labels('admin_labels.currency_code', 'Currency Code') }}
                                </label>
                                <input type="text" name="code" class="form-control" id="code" placeholder="{{ labels('admin_labels.placeholder_inr', 'inr') }}">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label for="">
                                    {{ labels('admin_labels.currency_exchange_rate', 'Currency Exchange Rate') }}
                                </label>
                                ...
                                <div class="col-md-12">
                                    <div class="form-group mb-2">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <label for="">
                                                {{ labels('admin_labels.exchange_rate_api_id', 'Exchange Rate API ID') }}
                                            </label>
                                            <button type="button" class="btn btn-sm btn-primary mb-4"
                                                data-bs-toggle="modal" data-bs-target="#currency_detail_modal">
                                                {{ labels('admin_labels.where_to_find_exchange_rate_api_id', 'Where to find exchange rate api id?') }}
                                            </button>
                                        </div>
                                            ...
                                            <div class="modal-dialog modal-lg modal-dialog-centered">
                                                <div class="modal-content">
                                                    <div class="modal-body">
                                                        <p>1. {{ labels('admin_labels.currency_api_step1', 'You have to sign up at this URL.') }} <a
                                                                href="https://openexchangerates.org/account/app-ids">{{ labels('admin_labels.url_text', 'URL') }}</a>.
                                                        </p>
                                                        <p>2. {{ labels('admin_labels.currency_api_step2', 'You will see the plans you can choose from. Opt for the Free Plan, which provides hourly updates (with the base currency USD) and up to 1,000 requests/month.') }} <a
                                                                href="https://openexchangerates.org/signup/free">{{ labels('admin_labels.free_plan', 'Free Plan') }}</a></p>
                                                        <p>3. {{ labels('admin_labels.currency_api_step3', 'After signing in, you will be able to see the App IDs menu in the sidebar.') }}</p>
                                                        <p>4. {{ labels('admin_labels.currency_api_step4', 'Click on that, and you will be able to see the app ID. Copy that ID and paste it into the input box provided below:') }}</p>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form enctype="multipart/form-data" method="POST" class="submit_form">
                    @method('PUT')
                    @csrf
                    <input type="hidden" class="edit_currency_id" name="edit_currency_id">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="code" class="form-control code" id=""
                                    placeholder="{{ labels('admin_labels.placeholder_inr', 'inr') }}">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label for="">
                                    {{ labels('admin_labels.currency_exchange_rate', 'Currency Exchange Rate') }}
                                ...
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="exportOptionsDropdown">
                                        <li><button class="dropdown-item" type="button"
                                                onclick="exportTableData('admin_currency_table','csv')">{{ labels('admin_labels.csv', 'CSV') }}</button></li>
                                        <li><button class="dropdown-item" type="button"
                                                onclick="exportTableData('admin_currency_table','json')">{{ labels('admin_labels.json', 'JSON') }}</button></li>
                                        <li><button class="dropdown-item" type="button"
                                                onclick="exportTableData('admin_currency_table','sql')">{{ labels('admin_labels.sql', 'SQL') }}</button></li>
                                        <li><button class="dropdown-item" type="button"
                                                onclick="exportTableData('admin_currency_table','excel')">{{ labels('admin_labels.excel', 'Excel') }}</button>
                                        </li>
                                    </ul>
                                </div>
                            </div>
