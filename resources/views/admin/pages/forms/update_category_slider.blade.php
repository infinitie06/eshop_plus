...
@section('content')
    ...
    @endphp
    <x-admin.breadcrumb :title="labels('admin_labels.update_categories_sliders', 'Update Categories Sliders')" :subtitle="labels(
                                ...
                                <form id="category-slider-form"
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="category_ids[]" required id="category_sliders_category"
                                            ...
                                            data-placeholder="{{ labels('admin_labels.type_to_search_and_select_categories', 'Type to search and select categories') }}">
...
document.addEventListener('DOMContentLoaded', function () {
    // Remove client-side required for Type (select/input named "type") and any file inputs
    function removeClientRequired() {
        document.querySelectorAll('input[name="type"], select[name="type"], input[type="file"], select[type="file"], input[name="banner_image"], input[name="file"]').forEach(function (el) {
            if (el.hasAttribute('required')) el.removeAttribute('required');
        });

        // remove 'required' from specific category select if you want (optional)
        var catSel = document.querySelector('#category_sliders_category');
        if (catSel && catSel.hasAttribute('required')) catSel.removeAttribute('required');
    }

    removeClientRequired();

    // Ensure required attributes are removed just before submit (some libs may re-add)
