...
<div class="message-card @if ($isSender) mc-sender @endif" data-id="{{ $id }}">
        ...
        @if (@$attachment->type != 'image' || $message)
            ...
            : nl2br($message) !!}
                ...
                @if (@$attachment->type == 'file')
                    <?php        $filename = 'download/' . basename($attachment->file); ?>
                    <a href="{{ $filename }}" class="file-download">
    ...
    <script>
        ...
        // Run manually after message send (hook this into Chatify's message-sending logic)
        function updateMessageTimes() {
            $('.message-time').each(function () {
                var timestamp = $(this).data('time');
                if (timestamp) {
                    var formatted = moment(timestamp).fromNow();
                    $(this).find('.time').text(formatted);
                }
            });
        }
    </script>