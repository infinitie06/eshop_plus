...
<div class="item col-item">
    @if ($details->type != 'combo-product')
        <div class="product-box {{ isset($details->store) && $details->store->status != 1 ? 'store-inactive' : '' }}">
            ...
            @endif
            <div class="product-image m-0 me-2">
                @php
                    $main_img = app(MediaService::class)->dynamic_image($details->image, 650);
                    $other_images = app(MediaService::class)->dynamic_image(
                        $details->other_images[0] ?? $details->image,
                        650,
                    );
                @endphp
                ...
                @else
                    <div class="button-action pt-2">
                        <div class="addtocart-btn add_cart" id="add_cart"
                            ...
                            data-brand-name='{!! app(TranslationService::class)->getDynamicTranslation(Brand::class, 'name', $details->brand, $language_code) !!}' data-image='{{ $main_img }}'
            ...
            <div class="product-details text-left">
                ...
                <div class="product-price">
                    @php
                        ...
                        $special_price = app(CurrencyService::class)->currentCurrencyPrice(
