    ...
    @if ($details->type != 'combo-product')
        <div class="product-box {{ isset($details->store) && $details->store->status != 1 ? 'store-inactive' : '' }}">
            ...
            <div class="product-image">
                @php
                    $main_img = app(MediaService::class)->dynamic_image($details->image, 650);
                    $other_images = app(MediaService::class)->dynamic_image($details->other_images[0] ?? $details->image, 650);
                @endphp
                <a wire:navigate href="{{ customUrl('products/' . $details->slug) }}"
                    ...
                    <img class="blur-up lazyload" src="{{ app(MediaService::class)->dynamic_image($details->image, 400) }}" alt="Product"
                ...
                <div class="button-set style2">
                    ...
                    @else
                        <div class="addtocart-btn add_cart" data-product-variant-id="{{ $details->variants[0]['id'] }}"
                            ...
                            data-image='{{ app(MediaService::class)->dynamic_image($details->image, 220) }}' data-product-type='regular'
        ...
        <div class="product-box {{ isset($details->store) && $details->store->status != 1 ? 'store-inactive' : '' }}">
            ...
            <div class="product-image">
                <a wire:navigate href="{{ customUrl('combo-products/' . $details->slug) }}"
                    ...
                    @php
                        $main_img = app(MediaService::class)->dynamic_image($details->image, 450);
                        $other_images = app(MediaService::class)->dynamic_image($details->other_images[0] ?? $details->image, 450);
                    @endphp
                ...
                <div class="button-set style2">
                    <div class="addtocart-btn add_cart" id="add_cart" data-product-variant-id="{{ $details->id }}"
                        ...
                        <a href="#quickview-modal" class="btn-icon quickview quick-view-modal" data-bs-toggle="modal"
                            data-bs-target="#quickview_modal" data-product-id="{{ $details->id }}"
                            tabindex="0"><span class="icon-wrap d-flex-justify-center h-100 w-100"
                    ...
                    <a href="#quickview-modal" class="btn-icon quickview quick-view-modal" data-bs-toggle="modal"
                        data-bs-target="#quickview_modal" data-product-type="combo-product" data-product-id="{{ $details->id }}">
                        <span class="icon-wrap d-flex-justify-center h-100 w-100" data-bs-toggle="tooltip"
                            data-bs-placement="top" title="" data-bs-original-title="Quick View"><i
                                class="hdr-icon icon anm anm-search-l"></i><span
                                class="text">{{ labels('front_messages.quick_view', 'Quick View') }}</span></span>
            ...
            <div class="product-details text-left">
                ...
                <div class="product-price">
                    @php
                        ...
                        $special_price = app(CurrencyService::class)->currentCurrencyPrice($details->special_price, true);
                    @endphp
