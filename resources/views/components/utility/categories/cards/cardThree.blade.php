...
<div class="swiper-slide slider-brand zoomscal-hov rounded-4">
    <a wire:navigate href="{{ customUrl('categories/' . $category->slug . '/products') }}"
        ...
        <div
            class="zoom-scal zoom-scal-nopb {{ $store_settings['category_card_style'] == 'category_card_style_1' ? 'img-box-rectangle-h150' : 'img-box-h140' }} {{ $store_settings['category_card_style'] == 'category_card_style_3' ? 'rounded-circle' : '' }}">
            <img class="blur-up lazyload" data-src="{{ $category->image }}" src="{{ $category->image }}"
                alt="{!! $category->name !!}" title="" /></div>
