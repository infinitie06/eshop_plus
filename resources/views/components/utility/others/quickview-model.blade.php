...
<div wire:ignore.self class="quickview-modal modal fade" id="quickview_modal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    @php
                                ...
                                @if ($this->product_type != 'combo-product')
                                    @if (!empty(app(TranslationService::class)->getDynamicTranslation(Brand::class, 'name', $product[0]->brand, $language_code)))
                                        <a wire:navigate
                                            ...
                                            class="text-ellipsis mb-10px" title="{!! app(TranslationService::class)->getDynamicTranslation(Brand::class, 'name', $product[0]->brand, $language_code) !!}"><ion-icon
                                                class="custom-icon fs-6 me-1"
                                                name="medal-outline"></ion-icon>{!! app(TranslationService::class)->getDynamicTranslation(Brand::class, 'name', $product[0]->brand, $language_code) !!}
                                        </a>
                                    @endif
                                    <a wire:navigate
                                        href="{{ customUrl('categories/' . $category[0]->slug . '/products') }}"
                                        class="text-ellipsis mb-10px text-secondary"
                                        title="{!! app(TranslationService::class)->getDynamicTranslation(Category::class, 'name', $product[0]->category_id, $language_code) !!}"><ion-icon name="layers-outline"
                                            class="custom-icon fs-6 me-1"></ion-icon>{!! app(TranslationService::class)->getDynamicTranslation(Category::class, 'name', $product[0]->category_id, $language_code) !!}
                                    </a>
                                @endif
                                <hr class="light-hr" />
                                    ...
                                    <div class="product-action d-flex-wrap w-100 pt-1 mb-3 clearfix">
                                        ...
                                        @php
                                            if ($this->product_type != 'combo-product') {
                                                if (count($product[0]->variants) <= 1) {
                                                    ...
                                                    $variant_price = $product[0]->variants[0]['special_price'] > 0 ? $product[0]->variants[0]['special_price'] : $product[0]->variants[0]['price'];
                                                } else {
                                                    $variant_id = '';
                                                    $variant_price = '';
                                                }
                                                ...
                                                $variant_price = $product[0]->special_price > 0 ? $product[0]->special_price : $product[0]->price;
                                            }
                                        @endphp
                                        <div class="addtocart ms-3 fl-1">
                                            <button type="submit" name="add"
                                                ...
                                                data-name='{{ htmlspecialchars($product[0]->name, ENT_QUOTES) }}'
                                                data-slug='{{ htmlspecialchars($product[0]->slug, ENT_QUOTES) }}'
                                                data-brand-name='{{ htmlspecialchars(app(TranslationService::class)->getDynamicTranslation(Brand::class, 'name', optional($product[0])->brand, $language_code) ?? '', ENT_QUOTES) }}'
                                                data-image='{{ htmlspecialchars($product[0]->image, ENT_QUOTES) }}'
                                                ...
                                                data-min='{{ $product[0]->minimum_order_quantity }}'
                                                data-variant-price="{{ $variant_price }}"><span>{{ labels('front_messages.add_to_cart', 'Add to cart') }}</span></button>
                                        </div>
                                    </div>
                                </div>
                                <!-- Social Sharing -->
