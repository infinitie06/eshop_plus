...
<nav class="navbar-vertical navbar bg-white" {{ session()->get('is_rtl') == 1 ? 'dir=rtl' : '' }}>
    <div class="nav-scroller bg-white">
        ...
        <div class="ps-2 pe-2">
            <!-- Search Bar -->
            <input type="text" class="form-control menuSearch"
        ...
        <ul class="navbar-nav" id="menuList">
            ...
            @if (($settings->store_mode ?? '') != 'single')
                <li class="nav-item ms-3">
                    <a class="nav-link {{ Request::is('admin/stores') || Request::is('admin/stores') ? 'active' : '' }}"
            ...
            <li class="nav-item ms-3">
                ...
                <div class="collapse {{ Request::is('admin/categories') || Request::is('admin/categories/*') ? 'show' : '' }}"
                    ...
                    <ul class="nav">
                        ...
                        <li class="nav-item ms-3">
                            <a class="nav-link {{ Request::is('admin/categories/bulk_upload') || Request::is('admin/categories/bulk_upload/*') || Request::is('categories/bulk_upload') ? 'active' : '' }}"
            ...
            <li class="sidebar-title ms-3"><i class='bx bx-chat'></i>
                {{ labels('admin_labels.affiliate', 'Affiliate System') }}
            </li>
            <li class="nav-item">
                <a class="nav-link {{ Request::is('admin/manage_affiliate_users') || Request::is('admin/add_affiliate_user') || Request::is('admin/affiliate_users*') ? 'active' : '' }}"
                    href="{{ route('admin.affiliate.manage_user') }}">
                    <span class="sidenav-normal">
                        {{ labels('admin_labels.affiliate_users', 'Affiliate Users') }}</span>
                </a>
                <a class="nav-link {{ Request::is('admin/affiliate_settings') ? 'active' : '' }}"
                    href="{{ route('admin.affiliate.settings') }}">
                    <span class="sidenav-normal">
                        {{ labels('admin_labels.settings', 'Settings') }}</span>
                </a>
                <a class="nav-link {{ Request::is('admin/affiliate/reports') ? 'active' : '' }}"
                    href="{{ route('admin.affiliate.reports') }}">
                    <span class="sidenav-normal">
                        {{ labels('admin_labels.reports', 'Reports') }}</span>
                </a>
                <a class="nav-link {{ Request::is('admin/affiliate_policies') ? 'active' : '' }}"
                    href="{{ route('admin.affiliate.policies') }}">
                    <span class="sidenav-normal">
                        {{ labels('admin_labels.policies', 'Policies') }}</span>
                </a>
            </li>
            <li class="sidebar-title ms-3"><i class='bx bx-card'></i>
            ...
            <li class="nav-item ms-3">
                ...
                <a class="nav-link {{ Request::is('admin/storage_type') || Request::is('admin/storage_type/*') ? 'active' : '' }}"
                    ...
                    <span class="nav-link-text ms-1">{{ labels('admin_labels.storage_type', 'Storage Type') }}</span>
                </a>
            </li>
            ...
            <li class="nav-item ms-3">
                ...
                <div class="collapse {{ Request::is('admin/web_settings*') ? 'show' : '' }}" id="web_setting_dropdown">
                    <ul class="nav">
                        <li class="nav-item">
                    ...
                    <ul class="nav">
                        <li class="nav-item">
                            <a class="nav-link {{ Request::is('admin/web_settings/pwa_settings') || Request::is('admin/web_settings/pwa_settings*') ? 'active' : '' }}"
                                ...
                                <span
                                    class="nav-link-text ms-1">{{ labels('admin_labels.pwa_settings', 'PWA Settings') }}</span>
                            </a>
                        </li>
                    </ul>
                    <ul class="nav">
            ...
            <li class="sidebar-title ms-3"><i class='bx bx-search-alt'></i>
                {{ labels('admin_labels.seo_management', 'SEO Management') }}
            </li>
            @if ($user_role == 'super_admin' || $logged_in_user->hasPermissionTo('view seo'))
                <li class="nav-item ms-3">
                    <a class="nav-link {{ Request::is('admin/seo') || Request::is('admin/seo/*') ? 'active' : '' }}"
                        href="{{ route('admin.seo.index') }}">
                        <span class="nav-link-text ms-1">{{ labels('admin_labels.seo', 'SEO') }}</span>
                    </a>
                </li>
            @endif
            ...
            <li class="nav-item ms-3">
                <a class="nav-link {{ Request::is('admin/reports/') || Request::is('admin/settings/sales_reports/*') ? 'active' : '' }}"
                    ...
                    <span class="nav-link-text ms-1">{{ labels('admin_labels.sales_reports', 'Sales Reports') }}</span>
                </a>
            </li>
        </ul>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function saveScrollPosition() {
            if (navScroller) {
                sessionStorage.setItem('sidebarScrollPos', navScroller.scrollTop);
            }
        }
        ...
        function restoreScrollPosition() {
            if (navScroller) {
                ...
                if (savedPos !== null) {
                    navScroller.scrollTop = parseInt(savedPos);
                } else if (activeLink) {
                    // If no saved position, scroll to active link
                    setTimeout(() => {
        ...
        collapsibleMenus.forEach(menu => {
            menu.addEventListener('click', function () {
                setTimeout(saveScrollPosition, 350); // Save after collapse animation
            });
        });

        // Auto-expand parent menu if active link is inside
        if (activeLink) {
            const parentCollapse = activeLink.closest('.collapse');
            if (parentCollapse) {
                const parentTrigger = document.querySelector(
                    `[data-bs-toggle="collapse"][data-bs-target="#${parentCollapse.id}"]`);
                if (parentTrigger) {
