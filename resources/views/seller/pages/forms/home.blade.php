...
@section('content')
    ...
    <section class="main-content">
            ...
            <div class="alert alert-warning alert-dismissible fade show" role="alert">
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-grow-1">
                        ...
                        <p class="mb-2">
                            {{ labels('admin_labels.please_create_store_to_continue', 'You don\'t have a store yet. Please create a store to continue using the seller panel.') }}
                        </p>
        ...
        @if ($misconfigured_products_count > 0)
            <div class="alert alert-warning alert-dismissible fade show" role="alert">
                <div class="d-flex align-items-center">
                    <i class='bx bx-error-circle me-2' style="font-size: 24px;"></i>
                    <div class="flex-grow-1">
                        <h5 class="alert-heading mb-1">
                            {{ labels('admin_labels.products_missing_configuration', 'Products Missing Configuration') }}</h5>
                        <p class="mb-2">
                            {{ $misconfigured_products_count }}
                            {{ $misconfigured_products_count == 1
                                ? labels('admin_labels.product_is_missing_shipping_config', 'product is missing required shipping configuration.')
                                : labels('admin_labels.products_are_missing_shipping_config', 'products are missing required shipping configuration.') }}
                            {{ labels('admin_labels.please_update_products_for_delivery', 'Please update your products to ensure they can be delivered.') }}
                        </p>
                        <a href="{{ route('seller.products.manage_product') }}" class="btn btn-warning btn-sm">
                            <i class='bx bx-edit me-1'></i>
                            {{ labels('admin_labels.view_products', 'View Products') }}
                        </a>
                    </div>
                </div>
            </div>
                    ...
                    <div class="row cols-5 d-flex">
                        ...
                        <div class="col col-xxl-3">
                            <div class="chart-card contact-list">
                                ...
                                <div class="listOfContacts mt-5 overflow-auto h-100"></div>
                            </div>
                        </div>
    ...
    <script>
        window.sellerDashboardText = {
            sold: @json(labels('admin_labels.sold', 'Sold')),
            reviews: @json(labels('admin_labels.reviews', 'Reviews')),
            rating: @json(labels('admin_labels.rating', 'Rating')),
        };
        $(document).ready(function() {
            setTimeout(function() {
                ...
                const data = {
                    Monthly: {
                        series: [{
                            name: @json(labels('admin_labels.sales', 'Sales')),
                            data: <?php echo json_encode($sales[0]['total_sale']); ?>
                        }, {
                            name: @json(labels('admin_labels.orders', 'Orders')),
                            data: <?php echo json_encode($sales[0]['total_orders']); ?>
                        }, {
                            name: @json(labels('admin_labels.revenue', 'Revenue')),
                    ...
                    tooltip: {
                        custom: function({
                            ...
                            return '<div><h6 class="text-center p-1"> ' + monthAbbreviation +
                                ' </h6> <hr><span class="p-2"> ' + (window.sellerDashboardText && window.sellerDashboardText.rating || 'Rating') + ': ' + ratingValue +
                                ' </span> </div>'
