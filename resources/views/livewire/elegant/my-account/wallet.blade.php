    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-12 col-sm-12 col-md-12 col-lg-9">
                ...
                <input type="hidden" name="logo" id="logo" value="{{ app(MediaService::class)->getImageUrl($web_settings['logo']) }}">
                            ...
                            <form action="{{ Route('wallet.refill') }}" method="post" id="wallet_refill_form">
                                ...
                                <div class="modal-body px-2 pt-2">
                                    ...
                                    <input type="number" name="add_amount" id="add_amount" min=1
                                        placeholder="{{ labels('front_messages.write_amount_to_add', 'Write The Amount You Want to Add') }}">
                            ...
                            <form action="{{ Route('wallet.withdrawal') }}" method="post" id="withdrawal_form">
                                ...
                                <div class="modal-body px-2 pt-2">
                                    ...
                                    <input type="number" name="withdrawal_amount" id="withdrawal_amount"
                                        placeholder="{{ labels('front_messages.write_amount_to_withdraw', 'Write The Amount You Want to Withdraw') }}"
                                        max="{{ $user_info['balance'] }}">
                                    <label for="withdrawal_amount"
                                        class="fw-500 fs-6">{{ labels('front_messages.payment_details', 'Payment details') }}</label>
                                    <textarea name="payment_address" id="payment_address" cols="30" rows="4" placeholder="{{ labels('front_messages.account_details', 'Account Details') }}"></textarea>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary"
                                        data-bs-dismiss="modal">{{ labels('front_messages.close', 'Close') }}</button>
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-field="order_id" data-sortable="false">
                                                {{ labels('front_messages.order_item_id', 'Order Item ID') }}
                                            </th>
...
@if (isset($payment_method->paypal_method) && $payment_method->paypal_method == 1)
    <script
        src="https://www.paypal.com/sdk/js?client-id={{ $payment_method->paypal_client_id }}&currency={{ $payment_method->currency_code ?? 'USD' }}">
    </script>
@endif
@if (isset($payment_method->paystack_method) && $payment_method->paystack_method == 1)
