    ...
    <div class="container-fluid h-100">
        <div class="orders-card mt-0 h-100">
            <div class="top-sec d-flex-justify-center justify-content-between mb-4">
                ...
                <a href="{{ route('front_end.orders.generatInvoicePDF', ['id' => $user_orders['order_data'][0]->id, 'user_id' => $user_orders['order_data'][0]->user_id]) }}"
                    class="btn btn-primary btn-sm instructions_files">
                    <i class='bx bx-download me-1'></i>{{ labels('admin_labels.invoice', 'Invoice') }}
                </a>
            ...
            <div class="row mt-2">
                <div class="col-sm-12">
                    ...
                    // Check if Shiprocket is the shipment method using multiple methods
                    $is_shiprocket = false;
                    if (!$tracking_data->isEmpty()) {
                    // Method 1: Check if shiprocket_order_id exists
                    $shiprocket_order_id = $tracking_data[0]['shiprocket_order_id'] ?? '';
                    if (!empty($shiprocket_order_id)) {
                    $is_shiprocket = true;
                    }

                    // Method 2: Check courier_agency contains 'shiprocket'
                    if (!$is_shiprocket) {
                    $courier_agency = $tracking_data[0]['courier_agency'] ?? '';
                    ...
                    <div class="row mt-3">
                        ...
                        <div class="col-lg-6 col-md-9 col-sm-8">
                                    ...
                                    <li>
                                        ...
                                        <div class="right">
                                            <span>
                                                ...
                                                {{ isset($currency_details) && !empty($currency_details) ? $currency_symbol . number_format((float) $user_order_item['price'], 2) : '' }}
                                                @else
                                    ...
                                    <li>
                                        ...
                                        <div class="right">
                                            <b class="text-black">
                                                @if ($is_shiprocket)
                                                {{-- Shiprocket - no currency conversion --}}
                                                {{ isset($currency_details) && !empty($currency_details) ? $currency_symbol . number_format((float) $user_order_item['main_price'], 2) : '' }}
                                                @else
                                                {{-- Standard shipping - apply currency conversion --}}
                                                {{ isset($currency_details) && !empty($currency_details) ? $currency_symbol . number_format((float) $user_order_item['main_price'] * $currency_details[0]->exchange_rate, 2) : '' }}
                                                @endif
                                            </b>
                                        </div>
                                    </li>
                            ...
                            foreach ($user_order_item['status'] as $statusEntry) {
                            if (isset($statusEntry[0]) && $statusEntry[0] === 'delivered') {
                            $deliveredTime = $statusEntry[1] ?? null;
                            break;
                            }
                            }

                            if (!empty($deliveredTime)) {
                            $deliveredDateTime = DateTime::createFromFormat('d-m-Y h:i:sa', $deliveredTime);
                            if ($deliveredDateTime !== false) {
                            $returnDeadline = $deliveredDateTime->modify('+' . $max_days_to_return_item . ' days');
                            ...
                            @endphp
                                ...
                                @if (
                                $user_order['type'] == 'digital_product' &&
                                $user_order_item['active_status'] == 'delivered' &&
                                !empty($user_order_item['download_allowed']) &&
                                !empty($user_order_item['download_link']))
                                <a href="{{ $user_order_item['download_link'] }}"
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    class="btn btn-primary btn-sm ms-2">
                                    <i class="bx bx-download me-1"></i>{{ labels('front_messages.download', 'Download') }}
                                </a>
                                @endif
                        </div>
                    </div>
                ...
                <div class="row">
                    <form action="{{ Route('orders.send-bank-receipt') }}" id="send_bank_receipt_form"
                        method="POST" enctype="multipart/form-data">

                        <input type="hidden" name="order_id"
                            value="{{ $user_orders['order_data'][0]->id }}">
                        <div class="form-group">
                            <label for="receipt">
                                <strong>{{ labels('front_messages.bank_payment_receipt', 'Bank Payment Receipt') }}</strong>
                            </label>
                            <input type="file" class="form-control" name="attachments[]"
                                id="receipt" multiple required />
                        </div>
                        <div class="form-group">
                            <button type="reset" class="btn btn-outline-warning btn-xs">
                                {{ labels('front_messages.reset', 'Reset') }}
                            </button>
                            <button type="submit" class="btn btn-outline-blue btn-xs" id="submit_btn">
                                {{ labels('front_messages.send', 'Send') }}
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="align-items-baseline col-md-6 d-flex gap-2 justify:spaceEvenly">
                        ...
                        <small>
                            [<a href="{{ customUrl('/storage/' . $row1['attachments']) }}"
                                target="_blank">
                                {{ labels('front_messages.attachment', 'Attachment') }}
                                {{ $i }}
                            </a>]
                        </small>
                        @php $i++; @endphp
                        ...
                        @php $status = $bank_transfer[0]['status']; @endphp
                        @if ($status == 0)
                        <label class="badge bg-warning">
                            {{ labels('front_messages.pending', 'Pending') }}
                        </label>
                        @elseif ($status == 1)
                        <label class="badge bg-danger">
                            {{ labels('front_messages.rejected', 'Rejected') }}
                        </label>
                        @elseif ($status == 2)
                        <label class="badge bg-primary">
                            {{ labels('front_messages.accepted', 'Accepted') }}
                        </label>
                        @else
                        <label class="badge bg-danger">
                            {{ labels('front_messages.invalid_value', 'Invalid Value') }}
                        </label>
                        @endif
                    </div>
                    @endif
                ...
                <div class="table-bottom-brd table-responsive">
                    <table class="table align-middle text-center order-table">
                        ...
                        <tbody>
                            ...
                            $rawDate = $status[1] ?? '';
                            if (!empty($rawDate)) {
                                foreach (['d-m-Y h:i:sa', 'd-m-Y H:i:s', 'Y-m-d h:i:sa', 'Y-m-d H:i:s', 'd-m-y h:i:sa'] as $fmt) {
                                    try {
                            ...
                            <tr>
                                ...
                                <td>{{ $date ? $date->format('d M Y') . ' ' . $date->format('h:i A') : $rawDate }}</td>
                            </tr>
                            @endforeach
                    ...
                    <div class="tracking-detail d-flex-center align-items-center mt-2">
                        <ul>
                            <li>
                                ...
                                <div class="right">
                                    <span {{-- class="text-uppercase">{{ $user_order['wallet_balance'] == $user_order['final_total'] ? 'Wallet' : ucwords(str_replace('_', ' ', $user_order['payment_method'])) }}</span> --}}
                                        class="text-uppercase">{{ ucwords(str_replace('_', ' ', $user_order['payment_method'])) }}</span>
                                </div>
                            </li>
                            <li>
                                <div class="left">
                                ...
                                <div class="right">
                                    <span>
                                        @if ($is_shiprocket)
                                        {{-- Shiprocket - no currency conversion --}}
                                        {{ isset($currency_details) && !empty($currency_details) ? $currency_symbol . number_format((float) $user_order['total'], 2) : '' }}
                                        @else
                                        {{-- Standard shipping - apply currency conversion --}}
                            ...
                            <li class="{{ $user_order['delivery_charge'] == 0 ? 'd-none' : '' }}">
                                ...
                                <div class="right">
                                    <span>
                                        @if ($is_shiprocket)
                                        {{-- Shiprocket - no currency conversion --}}
                                        +{{ isset($currency_details) && !empty($currency_details) ? $currency_symbol . number_format((float) $user_order['delivery_charge'], 2) : '' }}
                                        @else
                                        {{-- Standard shipping - apply currency conversion --}}
                            ...
                            <li class="{{ $user_order['promo_discount'] == 0 ? 'd-none' : '' }}">
                                ...
                                <div class="right">
                                    <span>
                                        @if ($is_shiprocket)
                                        {{-- Shiprocket - no currency conversion --}}
                                        -{{ isset($currency_details) && !empty($currency_details) ? $currency_symbol . number_format((float) $user_order['promo_discount'], 2) : '' }}
                                        @else
                                        {{-- Standard shipping - apply currency conversion --}}
                            ...
                            <li>
                                ...
                                <div class="right">
                                    <span>
                                        @if ($is_shiprocket)
                                        {{-- Shiprocket - no currency conversion --}}
                                        {{ isset($currency_details) && !empty($currency_details) ? $currency_symbol . number_format((float) $user_order['wallet_balance'], 2) : '' }}
                                        @else
                                        {{-- Standard shipping - apply currency conversion --}}
                            ...
                            <li>
                                ...
                                <div class="right text-black">
                                    <b>
                                        @if ($is_shiprocket)
                                        {{-- Shiprocket - no currency conversion --}}
                                        {{ isset($currency_details) && !empty($currency_details) ? $currency_symbol . number_format((float) $user_order['final_total'], 2) : '' }}
                                        @else
                                        {{-- Standard shipping - apply currency conversion --}}
            ...
            <div class="mb-2">
                <div class="top-sec d-flex-justify-center justify-content-between mb-4">
                    <h2 class="mb-0">{{ labels('front_messages.tracking_details', 'Tracking Details') }}
                    </h2>
                </div>
                <div class="tracking-detail d-flex-center align-items-center mt-2">
                    <ul>
                        <li>
                            <div class="left"><span>
                                    {{ labels('front_messages.courier_agency', 'Courier Agency') }} </span>
                            </div>
                            <div class="right text-black">
                                <span
                                    class="text-uppercase"><b>{{ $tracking_data[0]['courier_agency'] }}</b></span>
                            </div>
                        </li>
                        <li>
                            <div class="left"><span>
                            ...
                            <div class="right"><span>{{ $tracking_data[0]['tracking_id'] }}</span>
                            </div>
                        </li>
                        <li>
                            <div class="left">
                                <span>{{ labels('front_messages.url', 'URL') }}</span>
                            </div>
                            <div class="right">
                                <span>
                                    <a href="{{ $tracking_data[0]['url'] }}" target="_blank"
                                        rel="noopener noreferrer">
                                        {{ $tracking_data[0]['url'] }}
                                    </a>
                                </span>
