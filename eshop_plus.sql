...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 22, 2025 at 02:01 PM
...
CREATE TABLE `affiliate_trackings` (
  ...
  `affiliate_uuid` varchar(64) DEFAULT NULL,
  `product_id` bigint(20) UNSIGNED DEFAULT NULL,
  `product_type` varchar(64) DEFAULT NULL,
  `category_id` bigint(20) UNSIGNED DEFAULT NULL,
  `category_commission` decimal(10,2) DEFAULT 0.00,
  `token` varchar(512) NOT NULL,
  `usage_count` int(11) NOT NULL DEFAULT 0,
  `original_token` varchar(512) DEFAULT NULL,
...
CREATE TABLE `affiliate_users` (
  ...
  `affiliate_wallet_balance` decimal(10,2) NOT NULL DEFAULT 0.00,
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
...
CREATE TABLE `order_items` (
  ...
  `affiliate_token` varchar(256) DEFAULT NULL,
  `affiliate_commission` double(10,2) DEFAULT 0.00,
  `affiliate_commission_amount` decimal(10,2) DEFAULT 0.00,
  `is_affiliate_commission_settled` tinyint(4) NOT NULL DEFAULT 0,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
(28, 'admin_preference', '{\"store_mode\":\"multi\",\"order_notification\":0}', '2025-09-30 15:44:08', '2025-09-30 15:44:08');
...
(37, '1.1.2', '2024-06-06 06:38:08', '2024-06-06 06:38:08');
...
CREATE TABLE `users` (
  ...
  `is_affiliate_user` tinyint(4) DEFAULT 0,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` datetime NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
...
INSERT INTO `users` 
(`id`, `role_id`, `ip_address`, `username`, `password`, `email`, `mobile`, `image`, `disk`, `balance`, `activation_selector`, `activation_code`, `forgotten_password_selector`, `forgotten_password_code`, `forgotten_password_time`, `remember_selector`, `remember_token`, `created_on`, `last_login`, `active`, `company`, `address`, `bonus_type`, `bonus`, `cash_received`, `dob`, `country_code`, `city`, `area`, `street`, `pincode`, `serviceable_zipcodes`, `serviceable_cities`, `serviceable_zones`, `apikey`, `referral_code`, `friends_code`, `fcm_id`, `latitude`, `longitude`, `type`, `front_licence_image`, `back_licence_image`, `status`, `is_notification_on`, `is_affiliate_user`, `created_at`, `updated_at`, `active_status`, `avatar`, `dark_mode`, `messenger_color`) 
VALUES 
(1, 1, NULL, 'admin', '$2y$12$OyVep4C3a3xfjC/5365CO.SDyJlMYTSBlnk3VgXeAGP0dqgaUEs7m', 'admin@gmail.com', '9876543210', NULL, '', 0, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 1, NULL, NULL, 'percentage_per_order_item', NULL, 0.00, NULL, '91', NULL, NULL, NULL, NULL, NULL, '', NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'phone', NULL, NULL, 0, 1, 0, '2025-01-10 04:06:55', '2025-01-10 10:28:59', 0, 'avatar.png', 0, NULL);
-- --------------------------------------------------------
...
ALTER TABLE `categories`
ADD COLUMN `seller_id` BIGINT(20) UNSIGNED NOT NULL DEFAULT 0;

ALTER TABLE `sliders`
ADD COLUMN `web_image` VARCHAR(256) DEFAULT NULL;
...
ALTER TABLE `blogs`
ADD COLUMN `meta_title` VARCHAR(255) NULL AFTER `short_description`;
...
ALTER TABLE `products` ADD `pickup_location_id`  BIGINT NULL AFTER `hsn_code`;
ALTER TABLE `products` ADD CONSTRAINT `fk_products_pickup_location` FOREIGN KEY (`pickup_location_id`) REFERENCES `pickup_locations` (`id`) ON DELETE SET NULL;
-- Data migration query (conceptual if running on live DB):
-- UPDATE products p INNER JOIN pickup_locations pl ON p.pickup_location = pl.pickup_location SET p.pickup_location_id = pl.id;
ALTER TABLE `products` DROP COLUMN `pickup_location`;
ALTER TABLE `products` CHANGE COLUMN `pickup_location_id` `pickup_location`  BIGINT;

-- 2025_12_31_060135_add_pickup_location_id_to_parcels_table.php
ALTER TABLE `parcels` ADD `pickup_location` VARCHAR(255) NULL AFTER `store_id`;

-- 2026_01_22_040818_add_deep_link_fields_to_affiliate_tracking.php
ALTER TABLE `affiliate_trackings` ADD `click_count` INT DEFAULT 0 AFTER `usage_count`;
ALTER TABLE `affiliate_trackings` ADD `deep_link_url` TEXT NULL AFTER `original_token`;
ALTER TABLE `affiliate_trackings` ADD `web_url` TEXT NULL AFTER `deep_link_url`;
ALTER TABLE `affiliate_trackings` ADD `last_clicked_at` TIMESTAMP NULL AFTER `updated_at`;

-- 2026_01_22_100909_add_shipping_option_to_orders_table.php
ALTER TABLE `orders` ADD `shipping_option_id` VARCHAR(255) NULL AFTER `delivery_charge`;
ALTER TABLE `orders` ADD `shipping_option_name` VARCHAR(255) NULL AFTER `shipping_option_id`;
ALTER TABLE `orders` ADD `shipping_carrier` VARCHAR(255) NULL AFTER `shipping_option_name`;
ALTER TABLE `orders` ADD `shipping_estimated_days` VARCHAR(255) NULL AFTER `shipping_carrier`;

-- 2026_01_23_060053_add_is_shiprocket_order_to_orders_table.php


-- 2026_02_11_050000_create_seo_settings_table.php
CREATE TABLE `seo_settings` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `seo_type` enum('global','product','category','blog','page') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'global',
  ...
  `og_type` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'website',
  `twitter_card` enum('summary','summary_large_image') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'summary_large_image',
  ...
  `robots` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'index,follow',
  `structured_data` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin DEFAULT NULL CHECK (json_valid(`structured_data`)),
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `seo_settings_seo_type_reference_id_unique` (`seo_type`,`reference_id`),
  KEY `seo_settings_seo_type_reference_id_index` (`seo_type`,`reference_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
